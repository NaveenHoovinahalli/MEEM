...
public class Calenders extends SmartDataCategory {
    ...
    // Sql DB related
    private CalendarsDb mCalendarsDb;
    private AppLocalData mAppData = AppLocalData.getInstance();

    public Calenders(Context context, String inMirr, String inPlus, String outMirr, String outPlus) {
        super(context, Events.CONTENT_URI, null, inMirr, inPlus, outMirr, outPlus);
    ...
    // For Db Related
    public Calenders(Context context, String inMirr, String inPlus, String upid) {
        super(context, inMirr, inPlus);

        mContext = context;
        ArrayList<String> mtableNamesList = new ArrayList<String>();

        mtableNamesList.add(CalendarTableNames.CREATE_VAULT_LINKS_COUNT_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_VAULT_CALENDAR_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_CALENDAR_EVENT_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_CAL_COLOR_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_REMINDERS_LIST_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_RRULE_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_DAYSOFWEEK_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_DAYSOFMONTH_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_DAYSOFYEAR_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_WEEKOFYEAR_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_MONTHSOFYEAR_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_SETPOSITIONS_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_LIST_OF_ATTENDEES_TABLE);
        mtableNamesList.add(CalendarTableNames.CREATE_LIST_OF_INSTANCES_TABLE);

        mCalendarsDb = new CalendarsDb(context, upid, mAppData.getCalendarDbFullPath(), mtableNamesList);


        if (inMirr != null) {
            if (!mCalendarsDb.deleteCalendarForUpid(upid, 0)) dbgTrace("deleteCalendarForUpid Failed for upid" + upid + " For Mirr");
        }
        if (inPlus != null) {
            if (!mCalendarsDb.deleteCalendarForUpid(upid, 1)) dbgTrace("deleteCalendarForUpid Failed for upid" + upid + " For MirrPlus");
        }
    ...
    public Calenders(Context context) {
        super(context, Events.CONTENT_URI);
        mContext = context;
        mCalendarsDb = new CalendarsDb(context, mAppData.getCalendarDbFullPath());
    }
    ...
    long getNextMirrorItem() {
        ...
        try {
            ...
            eventObj = mReaderMapper.readValue(mMirrorParser, Event.class);
            dbgTrace("Ended");
            mCurrentMirrorItem = eventObj;
            dbgTrace("mirr item title: " + mCurrentMirrorItem.title + "\n checksum: " + mCurrentMirrorItem.checksum);
        ...
        return (eventObj != null) ? eventObj.checksum : -1;
    }
    ...
    long getNextMirrorPlusItem() {
        ...
        try {
            ...
            eventObj = mReaderMapper.readValue(mPlusParser, Event.class);
            dbgTrace("Ended");
            mCurrentPlusItem = eventObj;
    ...
    boolean addPhoneItemToMirror() {
        dbgTrace();
        if (mCurrentPhoneItem.deleted.equalsIgnoreCase("0")) {
    ...
    private boolean serialize(FileOutputStream fos, Event obj) {
        dbgTrace();
        try {
            mObjectWriter.withType(Event.class).writeValue(fos, obj);
            dbgTrace("Ended");
            return true;
    ...
    void addMirrToDataBase() {
        dbgTrace();
        mCalendarsDb.addToSqlDataBase(mCurrentMirrorItem, 0);
    }

    @Override
    void addMirrPlusToDataBase() {
        dbgTrace();
        mCalendarsDb.addToSqlDataBase(mCurrentPlusItem, 1);
    }

    private void dbgTrace() {
        GenUtils.logMethodToFile("Calenders.log");
    }

    private void dbgTrace(String trace) {
        GenUtils.logCat(tag, trace);
        GenUtils.logMessageToFile("Calenders.log", trace);
    }
    ...
    public boolean iterateNaddToPhoneDb(ArrayList<SmartDataInfo> smartDataInfoList) {
        ...
        for (SmartDataInfo sdInfo : smartDataInfoList) {
            ...
            mCurrentMirrorItem = mCalendarsDb.getEventForChecksum(sdInfo.getChecksum());
    ...
    public static class Event {
        ...
        public long crc32() {
            ...
            checksum.update(this.calendarDisplayName.getBytes(), 0, this.calendarDisplayName.length());
            checksum.update(this.dtstart.getBytes(), 0, this.dtstart.length());
            checksum.update(this.dtend.getBytes(), 0, this.dtend.length());
            checksum.update(this.title.getBytes(), 0, this.title.length());
            checksum.update(this.eventLocation.getBytes(), 0, this.eventLocation.length());
            checksum.update(this.description.getBytes(), 0, this.description.length());
            //checksum.update(this.rrule.getBytes(), 0, this.rrule.length());
