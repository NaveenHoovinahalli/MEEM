...
/**
 * Created by hercules on 16/9/16.
 */
public class CalendarsDb extends MeemSqlDBManager {
    private String tag = "CalendarDb";
    private Calenders.Event mCurrentItem;
    private String mUpid;


    public CalendarsDb(Context mContext, String uipd, String dbFilepath, ArrayList<String> tbNamesArrList) {
    ...
    public CalendarsDb(Context mContext, String dbFilepath) {
        super(mContext, dbFilepath);
        dbgTrace();
    }

    // To delete All Entries
    public CalendarsDb(Context context, String dbFilepath, String upid) {
        super(context, dbFilepath);
        mUpid = upid;
        dbgTrace();
    }

    private void dbgTrace() {
        GenUtils.logMethodToFile("CalendarSqlDb.log");
    }

    private void dbgTrace(String trace) {
        GenUtils.logCat(tag, trace);
        GenUtils.logMessageToFile("CalendarSqlDb.log", trace);
    }

    public void addToSqlDataBase(Calenders.Event currItem, int isMirr) {
        dbgTrace();
        ...
        if (!add(values, CalendarTableNames.VAULT_CALENDAR_TABLE)) {
            ...
            if (linkcount == 0) {
                if (addnewLink(mCurrentItem.checksum)) {
                    ...
                    if (!addCalendar_Event()) dbgTrace("Adding Calendar_event table Failed");

                    if (!addCal_Color()) dbgTrace("Adding Cal_color table Failed");

                    if (!addList_of_attendees()) dbgTrace("Adding List_of_attendees table Failed");

                    if (!addList_of_instances()) dbgTrace("Adding List_of_instances table Failed");

                    if (!addReminders_list()) dbgTrace("Adding Reminders_list table Failed");

                    if (!addRRule_list()) dbgTrace("Adding RRule_list table Failed");
    ...
    private int getLinkCountforCsum(int checkSum) {
        ...
        String querry = "select * from " + CalendarTableNames.LINKS_COUNT_TABLE + " where checksum = " + checkSum;
        return rawQuerryGetInt(querry, 1);
    }

    private boolean decrementLinkscountForChecksum(int checkSum) {
        dbgTrace();
        String sqlStmt = "update " + CalendarTableNames.LINKS_COUNT_TABLE +
    ...
    private boolean addnewLink(int checkSum) {
        ...
        return add(values, CalendarTableNames.LINKS_COUNT_TABLE);
    }
    ...
    public boolean deleteAll() {
        ...
        try {
            ...
            db.execSQL("delete from " + CalendarTableNames.LINKS_COUNT_TABLE);
            db.execSQL("delete from " + CalendarTableNames.VAULT_CALENDAR_TABLE);
            db.execSQL("delete from " + CalendarTableNames.CALENDAR_EVENT_TABLE);
            db.execSQL("delete from " + CalendarTableNames.REMINDERS_LIST_TABLE);
            db.execSQL("delete from " + CalendarTableNames.CAL_COLOR_TABLE);
            db.execSQL("delete from " + CalendarTableNames.RRULE_TABLE);
            db.execSQL("delete from " + CalendarTableNames.DAYSOFWEEK_TABLE);
            db.execSQL("delete from " + CalendarTableNames.DAYSOFMONTH_TABLE);
            db.execSQL("delete from " + CalendarTableNames.DAYSOFYEAR_TABLE);
            db.execSQL("delete from " + CalendarTableNames.WEEKOFYEAR_TABLE);
            db.execSQL("delete from " + CalendarTableNames.MONTHSOFYEAR_TABLE);
            db.execSQL("delete from " + CalendarTableNames.SETPOSITIONS_TABLE);
            db.execSQL("delete from " + CalendarTableNames.LIST_OF_ATTENDEES_TABLE);
            db.execSQL("delete from " + CalendarTableNames.LIST_OF_INSTANCES_TABLE);
    ...
    private boolean addCalendar_Event() {
        ...
        return add(values, CalendarTableNames.CALENDAR_EVENT_TABLE);
    ...
    private boolean addCal_Color() {
        ...
        ArrayList<Calenders.CalColor> calColorArrList = mCurrentItem.calColor;
        ...
        for (int i = 0; i < count; i++) {
            ...
            Calenders.CalColor mCalColor = calColorArrList.get(i);
            ...
            if (!add(values, CalendarTableNames.CAL_COLOR_TABLE)) {
                return false;
            }
    ...
    private boolean addList_of_attendees() {
        ...
        ArrayList<Calenders.AttendessBean> attendessBeanList = mCurrentItem.listofAttendess;
        ...
        for (int i = 0; i < count; i++) {
            ...
            Calenders.AttendessBean mAttendessBean = attendessBeanList.get(i);
            ...
            if (!add(values, CalendarTableNames.LIST_OF_ATTENDEES_TABLE)) {
                return false;
            }
    ...
    private boolean addList_of_instances() {
        ...
        ArrayList<Calenders.InstancesBean> instancesBeanList = mCurrentItem.listofInstances;
        ...
        for (int i = 0; i < count; i++) {
            ...
            Calenders.InstancesBean mInstancesBean = instancesBeanList.get(i);
            ...
            if (!add(values, CalendarTableNames.LIST_OF_INSTANCES_TABLE)) {
                return false;
            }
    ...
    private boolean addReminders_list() {
        ...
        ArrayList<Calenders.RemindersBean> remindersBeanList = mCurrentItem.listofReminders;
        ...
        for (int i = 0; i < count; i++) {
            ...
            Calenders.RemindersBean mRemindersBean = remindersBeanList.get(i);
            ...
            if (!add(values, CalendarTableNames.REMINDERS_LIST_TABLE)) {
                return false;
            }
    ...
    private boolean addRRule_list() {
        ...
        ArrayList<Calenders.Rrobj> RrobjList = mCurrentItem.rrObjs;
        ...
        for (int i = 0; i < count; i++) {
            ...
            Calenders.Rrobj mRrobj = RrobjList.get(i);
            ...
            if (!add(values, CalendarTableNames.RRULE_TABLE)) {
                return false;
            } else {
    ...
    private boolean addDays_of_week(Calenders.Rrobj currRrobj) {
        dbgTrace();

        ArrayList<Calenders.DaysOfTheWeek> daysOfWeekList = currRrobj.daysOfTheWeek;
        ...
        for (int i = 0; i < count; i++) {
            ...
            Calenders.DaysOfTheWeek mDaysOfweek = daysOfWeekList.get(i);
            ...
            if (!add(values, CalendarTableNames.DAYSOFWEEK_TABLE)) {
                return false;
            }
    ...
    private boolean addDays_of_Month(Calenders.Rrobj currRrobj) {
        dbgTrace();
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!add(values, CalendarTableNames.DAYSOFMONTH_TABLE)) {
                return false;
            }
    ...
    private boolean addDays_of_Year(Calenders.Rrobj currRrobj) {
        dbgTrace();
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!add(values, CalendarTableNames.DAYSOFYEAR_TABLE)) {
                return false;
            }
    ...
    private boolean addWeek_of_year(Calenders.Rrobj currRrobj) {
        dbgTrace();
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!add(values, CalendarTableNames.WEEKOFYEAR_TABLE)) {
                return false;
            }
    ...
    private boolean addMonths_of_year(Calenders.Rrobj currRrobj) {
        dbgTrace();
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!add(values, CalendarTableNames.MONTHSOFYEAR_TABLE)) {
                return false;
            }
    ...
    private boolean addSetPositions(Calenders.Rrobj currRrobj) {
        dbgTrace();
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!add(values, CalendarTableNames.SETPOSITIONS_TABLE)) {
                return false;
            }
    ...
    public Calenders.Event getEventForChecksum(int cSum) {
        Calenders.Event currDbItem = new Calenders.Event();
        currDbItem.checksum = cSum;

        if (!getCalendarEventTableForChecksum(currDbItem)) dbgTrace("getCalendarEventTableForChecksum " + cSum + "failed");
        if (!getCalColorTableForChecksum(currDbItem)) dbgTrace("getCalColorTableForChecksum " + cSum + "failed");
        if (!getReminderListTableForChecksum(currDbItem)) dbgTrace("getReminderListTableForChecksum " + cSum + "failed");
        if (!getRRuleTableForChecksum(currDbItem)) dbgTrace("getRRuleTableForChecksum " + cSum + "failed");
        if (!getListofAttendeesTableForChecksum(currDbItem)) dbgTrace("getListofAttendeesTableForChecksum " + cSum + "failed");
        if (!getListofInstancesTableForChecksum(currDbItem)) dbgTrace("getListofInstancesTableForChecksum " + cSum + "failed");
    ...
    private boolean getCalendarEventTableForChecksum(Calenders.Event currItem) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.CALENDAR_EVENT_TABLE + " where checksum = " + currItem.checksum;
    ...
    private boolean getCalColorTableForChecksum(Calenders.Event currItem) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.CAL_COLOR_TABLE + " where checksum = " + currItem.checksum;
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                Calenders.CalColor currCalcolor = new Calenders.CalColor();
    ...
    private boolean getReminderListTableForChecksum(Calenders.Event currItem) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.REMINDERS_LIST_TABLE + " where checksum = " + currItem.checksum;
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                Calenders.RemindersBean currRBean = new Calenders.RemindersBean();
    ...
    private boolean getRRuleTableForChecksum(Calenders.Event currItem) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.RRULE_TABLE + " where checksum = " + currItem.checksum;
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                Calenders.Rrobj currRrobj = new Calenders.Rrobj();
    ...
    private boolean getDaysoftheWeekTableForChecksum(Calenders.Rrobj robj, int cSum) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.DAYSOFWEEK_TABLE + " where checksum = " + cSum;
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                Calenders.DaysOfTheWeek currDaysoftheweek = new Calenders.DaysOfTheWeek();
    ...
    private boolean getDaysoftheMonthTableForChecksum(Calenders.Rrobj robj, int cSum) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.DAYSOFMONTH_TABLE + " where checksum = " + cSum;
    ...
    private boolean getDaysoftheYearTableForChecksum(Calenders.Rrobj robj, int cSum) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.DAYSOFYEAR_TABLE + " where checksum = " + cSum;
    ...
    private boolean getWeeksOftheYearTableForChecksum(Calenders.Rrobj robj, int cSum) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.WEEKOFYEAR_TABLE + " where checksum = " + cSum;
    ...
    private boolean getMonthsOftheYearTableForChecksum(Calenders.Rrobj robj, int cSum) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.MONTHSOFYEAR_TABLE + " where checksum = " + cSum;
    ...
    private boolean getSetpositionsTableForChecksum(Calenders.Rrobj robj, int cSum) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.SETPOSITIONS_TABLE + " where checksum = " + cSum;
    ...
    private boolean getListofAttendeesTableForChecksum(Calenders.Event currItem) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.LIST_OF_ATTENDEES_TABLE + " where checksum = " + currItem.checksum;
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                Calenders.AttendessBean currAttendee = new Calenders.AttendessBean();
    ...
    private boolean getListofInstancesTableForChecksum(Calenders.Event currItem) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.LIST_OF_INSTANCES_TABLE + " where checksum = " + currItem.checksum;
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                Calenders.InstancesBean currInstances = new Calenders.InstancesBean();
    ...
    public boolean deleteRowInAllOtherTablesForChecksum(int cSum) {
        ...
        try {
            ...
            db.execSQL("delete from " + CalendarTableNames.LINKS_COUNT_TABLE + " where checksum = " + cSum);
            db.execSQL("delete from " + CalendarTableNames.CALENDAR_EVENT_TABLE + " where checksum = " + cSum);
            db.execSQL("delete from " + CalendarTableNames.REMINDERS_LIST_TABLE + " where checksum = " + cSum);
            db.execSQL("delete from " + CalendarTableNames.CAL_COLOR_TABLE + " where checksum = " + cSum);
            db.execSQL("delete from " + CalendarTableNames.RRULE_TABLE + " where checksum = " + cSum);
            db.execSQL("delete from " + CalendarTableNames.DAYSOFWEEK_TABLE + " where checksum = " + cSum);
            db.execSQL("delete from " + CalendarTableNames.DAYSOFMONTH_TABLE + " where checksum = " + cSum);
            db.execSQL("delete from " + CalendarTableNames.DAYSOFYEAR_TABLE + " where checksum = " + cSum);
            db.execSQL("delete from " + CalendarTableNames.WEEKOFYEAR_TABLE + " where checksum = " + cSum);
            db.execSQL("delete from " + CalendarTableNames.MONTHSOFYEAR_TABLE + " where checksum = " + cSum);
            db.execSQL("delete from " + CalendarTableNames.SETPOSITIONS_TABLE + " where checksum = " + cSum);
            db.execSQL("delete from " + CalendarTableNames.LIST_OF_ATTENDEES_TABLE + " where checksum = " + cSum);
            db.execSQL("delete from " + CalendarTableNames.LIST_OF_INSTANCES_TABLE + " where checksum = " + cSum);
    ...
    public boolean deleteRowsInVaultsTableForUpid(String upid, int isMirr) {
        ...
        try {
            ...
            db.execSQL("delete from " + CalendarTableNames.VAULT_CALENDAR_TABLE + " where upid = '" + upid + "' and ismirr = " + isMirr);
            db.close();
        } catch (SQLException ex) {
            dbgTrace("SQLException: " + ex.getMessage());
            result = false;
    ...
    public boolean deleteCalendarForUpid(String upid, int isMirr) {
        boolean result = true;
        String sqlStmt = "select * from " + CalendarTableNames.VAULT_CALENDAR_TABLE + " where upid = '" + upid + "' and ismirr = " + isMirr;
        try {
    ...
    public boolean delteAllEntriesOfUpidInDb() {
        ...
        if (!deleteCalendarForUpid(mUpid, 0)) {
            dbgTrace("Failed! Delete Calendars of Upid " + mUpid + "Mirror Items");
            res = false;
        } else dbgTrace("Deleted All Calendars of Upid " + mUpid + "MirrorPlus Items");

        if (!deleteCalendarForUpid(mUpid, 1)) {
            dbgTrace("Failed! Delete Calendars of Upid " + mUpid + "MirrorPlus Items");
            res = false;
        } else dbgTrace("Deleted All Calendars of Upid " + mUpid + "MirrorPlus Items");
